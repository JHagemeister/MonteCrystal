...
public:
	...
	virtual std::string get_steps_header(void) = 0;
	/// header for output of mean values and heat capacity
	virtual std::string get_mean_header(void) = 0;
	...
	virtual std::string get_step_value(int index) = 0;
	/// get averaged observale values
	/** @param[in] temperature Temperature in [K] */
	virtual std::string get_mean_value(double temperature) = 0;
	/// clear storage and reset _measurement index
	virtual void clear_storage(void) = 0;

	void set_measurement_index(int measurementIndex);
	void set_number_measurements(int numberMeasurements);
	
	int get_measurement_index(void);
	int get_num_measurements(void);
