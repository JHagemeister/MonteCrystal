...
GUISimulationProcedureElements::GUISimulationProcedureElements(Ui::QtMainWindow* ui)
{
	/**
...
void GUISimulationProcedureElements::setup_elements(void)
{
	...
	*/
	...
	// simulation method
	_ui->comboBoxSimulationType->addItem((tr("Metropolis Monte Carlo")));
	_ui->comboBoxSimulationType->addItem((tr("LLG simulation")));
	_ui->comboBoxSimulationType->addItem((tr("Converger1")));

	std::vector<int> ratio;

	_ui->tableWidgetMagneticField->setHorizontalHeader(new MyHeaderView(Qt::Horizontal, 0.3, 0.25));
	_ui->tableWidgetMagneticField->setColumnCount(6);
	_ui->tableWidgetMagneticField->setRowCount(1);
	_ui->tableWidgetMagneticField->verticalHeader()->hide();
	ratio.push_back(76);
	ratio.push_back(76);
	ratio.push_back(76);
	ratio.push_back(40);
	ratio.push_back(40);
	ratio.push_back(40);
	_ui->tableWidgetMagneticField->set_column_ratio(ratio);
	_ui->tableWidgetMagneticField->set_horizontal_header_height(5);
	ratio.clear();
	ratio.push_back(4);
	_ui->tableWidgetMagneticField->set_row_ratio(ratio);
	for (int i = 0; i < 6; i++)
	{
		_ui->tableWidgetMagneticField->setItem(0,i, new QTableWidgetItem());
	}

	_ui->tableWidgetTemperature->setHorizontalHeader(new MyHeaderView(Qt::Horizontal, 0.2, 0.25));
	_ui->tableWidgetTemperature->setColumnCount(6);
	_ui->tableWidgetTemperature->setRowCount(1);
	_ui->tableWidgetTemperature->verticalHeader()->hide();
	ratio.clear();
	ratio.push_back(76);
	ratio.push_back(76);
	ratio.push_back(76);
	...
	ratio.push_back(40);
	ratio.push_back(40);
	_ui->tableWidgetTemperature->set_column_ratio(ratio);
	_ui->tableWidgetTemperature->set_horizontal_header_height(45);
	ratio.clear();
	ratio.push_back(40);
	_ui->tableWidgetTemperature->set_row_ratio(ratio);
	for (int i = 0; i < 6; i++)
	{
		_ui->tableWidgetTemperature->setItem(0, i, new QTableWidgetItem());
	}

	_ui->tableWidgetSimulation->setColumnCount(3);
	_ui->tableWidgetSimulation->setRowCount(1);
	_ui->tableWidgetSimulation->verticalHeader()->hide();
	_ui->tableWidgetSimulation->setHorizontalHeaderLabels(QString("seed;SimSteps;outWidth;").split(";"));
	ratio.clear();
	ratio.push_back(100);
	ratio.push_back(134);
	ratio.push_back(115);
	_ui->tableWidgetSimulation->set_column_ratio(ratio);
	_ui->tableWidgetSimulation->set_horizontal_header_height(4);
	ratio.clear();
	ratio.push_back(4);
	_ui->tableWidgetSimulation->set_row_ratio(ratio);
	for (int i = 0; i < 3; i++)
	{
		_ui->tableWidgetSimulation->setItem(0, i, new QTableWidgetItem());
	}
}

void GUISimulationProcedureElements::read_parameters(QSharedPointer<Configuration> &config)
{
	if (_ui->comboBoxSimulationType->currentText().compare("Metropolis Monte Carlo") == 0)
	{
		config->_simulationType = metropolis;
	}
	else if (_ui->comboBoxSimulationType->currentText().compare("LLG simulation") == 0)
	{
		config->_simulationType = landauLifshitzGilbert;
		if (_ui->tableWidgetSimulation2->item(0, 0))
		{
			double LLG_dampingParameter = _ui->tableWidgetSimulation2->item(0, 0)->text().toDouble();
			config->_LLG_dampingParameter = LLG_dampingParameter;
		}
		if (_ui->tableWidgetSimulation2->item(0, 1))
		{
			double LLG_timeWidth = _ui->tableWidgetSimulation2->item(0, 1)->text().toDouble();
			config->_LLG_timeWidth = LLG_timeWidth;
		}
	}
	else if (_ui->comboBoxSimulationType->currentText().compare("Converger1") == 0)
	{
		config->_simulationType = converger1;
	}

	if (_ui->tableWidgetTemperature->item(0, 0))
	{
		config->_temperatureStart = _ui->tableWidgetTemperature->item(0, 0)->text().toDouble();
	}
	if (_ui->tableWidgetTemperature->item(0, 1))
	{
		config->_temperatureEnd = _ui->tableWidgetTemperature->item(0, 1)->text().toDouble();
	}
	if (_ui->tableWidgetTemperature->item(0, 2))
	{
		config->_temperatureSteps = _ui->tableWidgetTemperature->item(0, 2)->text().toInt();
	}
	if (_ui->tableWidgetTemperature->item(0, 3) && _ui->tableWidgetTemperature->item(0, 4)
		&& _ui->tableWidgetTemperature->item(0, 5))
	{
		Threedim direction = { 0,0,0 };
		direction.x = _ui->tableWidgetTemperature->item(0, 3)->text().toDouble();
		direction.y = _ui->tableWidgetTemperature->item(0, 4)->text().toDouble();
		direction.z = _ui->tableWidgetTemperature->item(0, 5)->text().toDouble();
		config->_temperatureGradientDirection = direction;
	}

	if (_ui->tableWidgetSimulation->item(0, 0))
	{
		config->_seed = _ui->tableWidgetSimulation->item(0, 0)->text().toInt();
	}
	if (_ui->tableWidgetSimulation->item(0, 1))
	{
		config->_simulationSteps = _ui->tableWidgetSimulation->item(0, 1)->text().toInt();
	}
	if (_ui->tableWidgetSimulation->item(0, 2))
	{
		config->_outputWidth = _ui->tableWidgetSimulation->item(0, 2)->text().toInt();
	}
}
...
void GUISimulationProcedureElements::on_change_in_simulation_type(QString qString)
{
	...
	{
		_ui->tableWidgetSimulation2->setColumnCount(2);
		_ui->tableWidgetSimulation2->setRowCount(1);
		_ui->tableWidgetSimulation2->verticalHeader()->hide();
		_ui->tableWidgetSimulation2->custom_resize();
		QStringList header;
		header << QChar(945) << QString(QChar(916)) + "t [ps]";
		_ui->tableWidgetSimulation2->setHorizontalHeaderLabels(header);
		...
		_ui->tableWidgetSimulation2->setItem(0, 0, itemDampingParameter);

		QTableWidgetItem* itemTimeWidth = new QTableWidgetItem();
		itemTimeWidth->setText(QString::number(_llgSimParameters->deltaT));
		_ui->tableWidgetSimulation2->setItem(0, 1, itemTimeWidth);


		_ui->tableWidgetSimulation->item(0, 1)->setText(QString::number(_llgSimParameters->simulationSteps));
		_ui->tableWidgetSimulation->item(0, 2)->setText(QString::number(_llgSimParameters->outWidth));
		if (_ui->tableWidgetUIUpdate->item(0, 0))
		{
			_ui->tableWidgetUIUpdate->item(0, 0)->setText(QString::number(_llgSimParameters->uiWidth));
		}
	}
	else if (qString.compare("Metropolis Monte Carlo") == 0)
	{
		_ui->tableWidgetSimulation->item(0, 1)->setText(QString::number(_monteCarloSimParameters->simulationSteps));
		_ui->tableWidgetSimulation->item(0, 2)->setText(QString::number(_monteCarloSimParameters->outWidth));

		_ui->tableWidgetSimulation2->clear();
		_ui->tableWidgetSimulation2->setColumnCount(0);
		
		if (_ui->tableWidgetUIUpdate->item(0, 0))
		{
			_ui->tableWidgetUIUpdate->item(0, 0)->setText(QString::number(_monteCarloSimParameters->uiWidth));
		}
	}
	else if (qString.compare("Converger1") == 0)
	{
		_ui->tableWidgetSimulation->item(0, 1)->setText(QString::number(_converger1SimParameters->simulationSteps));
		_ui->tableWidgetSimulation->item(0, 2)->setText(QString::number(_converger1SimParameters->outWidth));
		...
		if (_ui->tableWidgetUIUpdate->item(0, 0))
		{
			_ui->tableWidgetUIUpdate->item(0, 0)->setText(QString::number(_converger1SimParameters->uiWidth));
		}
	}
}
...
e<font size=4><sub>y</sub>;e<font size=4><sub>z</sub>");
		_ui->tableWidgetMagneticField->setHorizontalHeaderLabels(header.split(";"));
		...
		ratio.push_back(40);
		_ui->tableWidgetTemperature->set_column_ratio(ratio);
		_ui->tableWidgetTemperature->custom_resize();
		_ui->tableWidgetTemperature->setHorizontalHeaderLabels(QString(
...
<i>T</i><font size=4><sub>1 </sub></font size=4>[K];steps;X;X;X").split(";"));
	...
	if (qString.compare("spin-seebeck") == 0)
	{
		...
		_ui->tableWidgetTemperature->set_column_ratio(ratio);
		_ui->tableWidgetTemperature->custom_resize();
		_ui->tableWidgetTemperature->setHorizontalHeaderLabels(
...
void GUISimulationProcedureElements::on_change_in_magnetic_field_table(int row, int column)
{
	if (_ui->tableWidgetMagneticField->item(row, column))
	{
		...
		if (_ui->comboBoxSimulationType->currentText().compare("LLG simulation") == 0)
		{
			magneticFieldParameters = _llgMagneticFieldParameters;
		}
		if (_ui->comboBoxSimulationType->currentText().compare("Converger1") == 0)
		{
			magneticFieldParameters = _converger1MagneticFieldParameters;
		}
		if (magneticFieldParameters != NULL)
		{
			double value = _ui->tableWidgetMagneticField->item(row, column)->text().toDouble();
			switch (column)
			{
...
void GUISimulationProcedureElements::on_change_in_temperature_table(int row, int column)
{
	if (_ui->tableWidgetTemperature->item(row, column))
	{
		...
		if (temperature != NULL)
		{
			double value = _ui->tableWidgetTemperature->item(row, column)->text().toDouble();
			switch (column)
			{
...
void GUISimulationProcedureElements::on_change_in_simulation_table(int row, int column)
{
	if (_ui->tableWidgetSimulation->item(row, column))
	{
		...
		if (parameters != NULL)
		{
			double value = _ui->tableWidgetSimulation->item(row, column)->text().toDouble();
			switch (column)
			{
...
void GUISimulationProcedureElements::on_change_in_simulation_table_two(int row, int column)
{
	if (_ui->tableWidgetSimulation2->item(row, column))
	{
		...
		if (parameters != NULL)
		{
			double value = _ui->tableWidgetSimulation2->item(row, column)->text().toDouble();
			switch (column)
			{
...
void GUISimulationProcedureElements::on_change_in_ui_update_table(int row, int column)
{
	if (_ui->tableWidgetUIUpdate->item(row, column))
	{
		...
		if (parameters != NULL)
		{
			double value = _ui->tableWidgetUIUpdate->item(row, column)->text().toDouble();
			switch (column)
			{
...
void GUISimulationProcedureElements::set_gui_magnetic_field_elements(QString qString)
{
	...
	if (parameters != NULL)
	{
		_ui->tableWidgetMagneticField->item(0, 0)->setText(QString::number(parameters->start));
		_ui->tableWidgetMagneticField->item(0, 1)->setText(QString::number(parameters->end));
		_ui->tableWidgetMagneticField->item(0, 2)->setText(QString::number(parameters->steps));
		_ui->tableWidgetMagneticField->item(0, 3)->setText(QString::number(parameters->direction.x));
		_ui->tableWidgetMagneticField->item(0, 4)->setText(QString::number(parameters->direction.y));
		_ui->tableWidgetMagneticField->item(0, 5)->setText(QString::number(parameters->direction.z));
	}
}
...
void GUISimulationProcedureElements::set_gui_temperature_elements(QString qString)
{
	...
	if (temperature != NULL)
	{
		...
		if (qString.compare("temperature-magnetic-field-loop") == 0)
		{
			_ui->tableWidgetTemperature->item(0, 0)->setText(QString::number(temperature->start));
			_ui->tableWidgetTemperature->item(0, 1)->setText(QString::number(temperature->end));
			_ui->tableWidgetTemperature->item(0, 2)->setText(QString::number(temperature->steps));
			_ui->tableWidgetTemperature->item(0, 3)->setText("");
			_ui->tableWidgetTemperature->item(0, 4)->setText("");
			_ui->tableWidgetTemperature->item(0, 5)->setText("");
		}
		if (qString.compare("spin-seebeck") == 0)
		{
			_ui->tableWidgetTemperature->item(0, 0)->setText(QString::number(temperature->start));
			_ui->tableWidgetTemperature->item(0, 1)->setText(QString::number(temperature->end));
			_ui->tableWidgetTemperature->item(0, 2)->setText(QString::number(temperature->steps));
			_ui->tableWidgetTemperature->item(0, 3)->setText(QString::number(temperature->direction.x));
			_ui->tableWidgetTemperature->item(0, 4)->setText(QString::number(temperature->direction.y));
			_ui->tableWidgetTemperature->item(0, 5)->setText(QString::number(temperature->direction.z));
		}
	}
}
